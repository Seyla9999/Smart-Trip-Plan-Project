  ...
  <section class="section">
    ...
    <div v-else class="results-grid">
      <div
        ...
        class="rc"
        @click="$router.push(item.id ? `/attractions/${item.id}` : `/discover`)"
      >
        <div
          class="rc-img"
...
const FALLBACK_DATA: Record<string, DisplayItem[]> = {
  Sea: [
    ...
    { name: 'Green Cathedral',   province: 'Sihanoukville', image: 'https://images.unsplash.com/photo-1511497584788-876760111969?w=500&q=80', color: '#2D6A4F', rating: '★★★★☆', reviews: 145 },
    { name: 'Boum Sakor Park',   province: 'Koh Kong',      image: 'https://images.unsplash.com/photo-1564760055775-d63b17a55c44?w=500&q=80', color: '#3E6B47', rating: '★★★★☆', reviews: 87  },
  ],
  Waterfall: [
  ...
  Temple: [
    ...
    { name: 'Preah Vihear',      province: 'Preah Vihear',  image: 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&q=80', color: '#6B4C3B', rating: '★★★★☆', reviews: 543  },
    { name: 'Banteay Srei',      province: 'Siem Reap',     image: 'https://images.unsplash.com/photo-1538964173425-93884e739ccd?w=500&q=80', color: '#7A6030', rating: '★★★★☆', reviews: 876  },
  ],
  Nature: [
    ...
    { name: 'Bokor National Park',province: 'Kampot',       image: 'https://images.unsplash.com/photo-1540541338287-41700207dee6?w=500&q=80', color: '#5C7A3E', rating: '★★★★★', reviews: 445 },
  ...
  Pagoda: [
    ...
    { name: 'Phnom Chisor',      province: 'Takéo',         image: 'https://images.unsplash.com/photo-1580674285054-bed31e145f59?w=500&q=80', color: '#6B7C3E', rating: '★★★☆☆', reviews: 156 },
  ],
  Culture: [
    ...
    { name: 'Tuol Sleng Museum', province: 'Phnom Penh',    image: 'https://images.unsplash.com/photo-1598946329549-8ac25a6c2890?w=500&q=80', color: '#6B4C3B', rating: '★★★★☆', reviews: 1234 },
    { name: 'Angkor Thom',       province: 'Siem Reap',     image: 'https://images.unsplash.com/photo-1538964173425-93884e739ccd?w=500&q=80', color: '#4A7C59', rating: '★★★★★', reviews: 987  },
    { name: 'Kampong Phluk',     province: 'Siem Reap',     image: 'https://images.unsplash.com/photo-1538964173425-93884e739ccd?w=500&q=80', color: '#2196A6', rating: '★★★★☆', reviews: 432  },
  ],
}
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function mapAttraction(a: Attraction): DisplayItem {
      const rating = Math.round(Number(a.average_rating) || 0)
      return {
        id:       a.id,
        name:     a.name_en,
        province: a.province?.name_en ?? '',
        image:    a.province?.main_image_url || 'https://images.unsplash.com/photo-1538964173425-93884e739ccd?w=500&q=80',
        color:    '#4A7C59',
        rating:   STAR_MAP[Math.min(rating, 5)] || '★★★☆☆',
        reviews:  0,
      }
    }

    async function fetchCategory(cat: string) {
      if (!props.fetchAttractions) {
        liveItems.value = []
        return
      }
      catLoading.value = true
      try {
        const results = await props.fetchAttractions(cat)
    ...
    watch(activeProv, async (prov) => {
      if (!props.fetchAttractions) return
      catLoading.value = true
      try {
        const results = await props.fetchAttractions(activeCat.value, prov === 'All' ? undefined : prov)
        liveItems.value = results.map(mapAttraction)
      } catch {
        liveItems.value = []
      } finally {
...
.see-all-link { font-size: 14px; color: #F4D58D; text-decoration: none; border-bottom: 1px solid rgba(244,213,141,0.40); padding-bottom: 2px; transition: border-color 0.2s; }
