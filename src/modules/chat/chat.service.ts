...
export class ChatService {
  ...
  async sendMessage(
    ...
    try {
      const senderName = msg[0]?.sender_name || 'Someone'
      const notifMsg   = imageUrl && !text?.trim()
        ? `${senderName} sent you a photo`
        : text?.trim() || ''
      const otherMembers = await this.convRepo.manager.query(
      ...
      for (const member of otherMembers) {
        await this.convRepo.manager.query(
          `INSERT INTO notifications (user_id, type, title, message, link, is_read)
           VALUES ($1, 'new_message', $2, $3, '/chat', false)`,
          [member.user_id, senderName, notifMsg],
        )
      }
    } catch { /* notification failure never breaks message sending */ }
  ...
  async updateConversation(
    ...
    if (updates.name) conv.name = updates.name
    if (updates.avatar) conv.avatar = updates.avatar
